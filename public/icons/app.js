...
function renderAssignmentGroup(title, type) {
  ...
  return `<h3>${title}</h3>${players.map(p => {
    ...
    return `<div class="assignment-row">
      <div><div class="person-name">${playerName(p)}</div><div class="person-meta">${baseLabel}${roleLabel}</div></div>
      <div class="assignment-actions">
        <select class="position-select" data-change="match-position" data-player="${p.id}" aria-label="Position de ${escAttr(playerName(p))}" ${locked || team==='absent'?'disabled':''} title="${team==='absent'?'Assigne d’abord le joueur à une équipe':'Position pour ce match'}">
          <option value="skater" ${position==='skater'?'selected':''}>Joueur</option>
          <option value="goalie" ${position==='goalie'?'selected':''}>Gardien</option>
        </select>
